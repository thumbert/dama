...
void testSplineInterpolator() {
  test('spline, 2 segments degenerate linear', () {
    var ipolator = SplineInterpolator([0, 0.5, 1], [0, 0.5, 1]);
    expect(ipolator.valueAt(0.4), 0.4);
  });

  test('spline, 3 segments degenerate linear', () {
    var ipolator = SplineInterpolator([0, 0.5, 1, 1.5], [0, 0.5, 1, 1.5]);
  ...
  test('spline, general', () {
    var xData = List.generate(9, (i) => i + 1);
    var yData = xData.map((x) => (x - 6) * (x - 6)).toList();
    var iPolator = SplineInterpolator(xData, yData);

    var x = [1.04, 2.44, 3.52];
    var yAct = ['24.62311983505155', '12.64779150515464', '6.156678597938145'];
    expect(x.map((e) => iPolator.valueAt(e)!.toStringAsPrecision(16)).toList(),
        yAct);
  });
}
