...
class Autocorrelation {
  ...
  Autocorrelation(this.data, this.maxLag) {
    ...
    _autocorrelation = List<num>(maxLag);
  ...
  num confidenceInterval({num confidenceInterval: 0.95, InputType inputType: InputType.whiteNoise}) {
    num out;
    if (inputType == InputType.whiteNoise) {
      out = GaussianDistribution().quantile((1 + confidenceInterval)/2)/sqrt(data.length);
    } else {
