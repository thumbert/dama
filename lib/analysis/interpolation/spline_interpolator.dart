...
class SplineInterpolator {
  ...
  SplineInterpolator(this.xData, this.yData) {
    ...
    List<num> mu = new List(n);
    List<num> z = new List(n + 1);
    mu[0] = 0.0;
    z[0] = 0.0;
    num g = 0.0;
  ...
  int _findInd(num x) {
    int i;
    if (x < xData.first) return 0;
    else if (x > xData.last) return xData.length-1;
    else {
  ...
  num derivativeAt(num x, {int order: 1}) {
    ...
    if (order < 1)
      throw new ArgumentError('Order needs to be >= 1');
